...
void solve(int W, int N, vector<long long[3]>& A) {
    vector<long long> initVals(W+1, -1);
    initVals[0] = 0;
    SegmentTree<long long> seg(initVals, -1, [](long long a, long long b) {return max(a, b);});
    for (int i=0; i<N; i++) {
        auto [l, r, v] = A[i];
        for (int j=W; j>=l; j--) {
            long long val = seg.query(j-r, j-l+1);
            if (val != -1) {
                seg.update(j, max(val+v, seg.query(j, j+1)));
            }
        }
    }
    cout << seg.query(W, W+1) << endl;
}

int main() {
    int W, N;
    cin >> W >> N;
    vector<long long[3]> A(N);
    for (int i=0; i<N; i++) {
