...
int main(){
    int N, K;
    cin >> N >> K;
    mint p1 = modpow(mint(N), 2);
    for(int i = 0; i < K; i++){
        auto a = (modpow(mint(N), 2)-p1)*2;
        auto b = p1*((modpow(mint(N-1), 2)+1));
        p1 = (modpow(mint(N), 2)-p1)*2 + p1*((modpow(mint(N-1), 2)+1));
        p1 /= modpow(mint(N), 2);
    }
    mint ans{p1};
    mint po = (modpow(mint(N), 2) - p1)/(N-1);
    

    cout << (p1 + (mint(N)*mint(N+1)/2 - 1)*po)/modpow(mint(N), 2) << endl;
}