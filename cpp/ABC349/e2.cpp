...
int check_agari(int grid[3][3], const ll point_grid[3][3]) {
    for (int i=0; i<3; i++) {
        // よこ
...
bool can_win(int grid[3][3], int first, const ll point_grid[3][3]) {
    // print_grid();
    int second;
    if (first == 1) {
        second = 2;
    ...
    auto tmp = check_agari(grid, point_grid);
    if (tmp == first) {
        // print_grid(grid);
        return true;
    } else if (tmp == second){
    ...
    for (int i=0; i<3; i++) {
        for (int j=0; j<3; j++) {
            if (grid[i][j] == 0) {
                grid[i][j] = first;
                if (!can_win(grid, second, point_grid)) {
                    grid[i][j] = 0;
                    return true;
                }
                grid[i][j] = 0;
